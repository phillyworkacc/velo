...
import { CircleCheck, Notebook } from "lucide-react";

export default function EditorTasks ({ editorTasks, tasksCompletedByEditor }: { editorTasks: EditorTasks[], tasksCompletedByEditor: string[] }) {
            ...
            {editorTasks[dateKey].map((editorTask, index) => (
               <Card key={index} className="pd-2 pdx-2" maxWidth="500px" cursor onClick={() => router.push(`/editor/editor-task/${editorTask.taskId}`)} hoverAnimate analyticsCard>
                  ...
                  <div className="text-s dfb align-center gap-10 mt-1 ">
                     <div className="text-xxs fit accent-color bold-600 dfb align-center gap-5">
                        <Notebook size={16} /> Editor Task
                     </div>
                     {(tasksCompletedByEditor.includes(editorTask.taskId)) && (<div className="text-xxs fit bold-600 dfb align-center gap-5" style={{color:"#1b8500ff"}}>
